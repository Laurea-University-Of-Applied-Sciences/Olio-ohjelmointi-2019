...
public class MultipleScreens extends JFrame {
	// If Eclipse shows a warning about "serializable Class does not declare a
	// static final serialVersionUID ...", just ignore it
	...
	/*
	 * myDB is an Array of car objects with fixed size defined by the value stored
	 * in MAX_QTY. It is used as a Class variable as a workaround to simulate a
	 * database. A similar, more flexible workaround could be implemented with the
	 * use of an ArrayList. The final and proper solution is to use a database to
	 * store data about the car objects
	 */
	...
	public MultipleScreens() {
		...
		getContentPane().setLayout(null); // Sets the JFrame's container with a null Layout Manager (Absolute
											// positioning)
		...
		tableCar.setModel(new DefaultTableModel(new Object[MAX_QTY][3], // MAX_QTY rows, 3 columns
				new String[] { "Plate Number", "Colour", "Model" } // Column headers (will not be displayed due to
																	// JTable limitations)
		));
		...
		btnAddCar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// Open a standard input dialog to get input from the user and insert data into
				// myDB
				getNewCarFromUser();
	...
	// Populate a JTable (tableCar) with data from my Car database (i.e the Array
	// containing car objects)
	...
	// This method creates two cars with some "hard-coded" values and stores them in
	// the myDB array
	...
	// This method will display the dialog box to get the input from the user and
	// store the new car into the myDB array
	private void getNewCarFromUser() {
		...
		// Displaying the input dialog: a standard Confirmation dialog showing the text
		// fields from the JPanel myPanel
		int result = JOptionPane.showConfirmDialog(null, myPanel, "Enter details of your new car",
				JOptionPane.OK_CANCEL_OPTION);

		if (result == JOptionPane.OK_OPTION) {
			// Update myDB by adding a new car. Note that dbItems has a value equivalent to
			// the next free index in myDB
